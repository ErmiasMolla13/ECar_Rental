...
// Clicked directly from the email link.
export async function GET(request) {
  const token = request.nextUrl.searchParams.get('token');
  const result = await verifyEmailToken('customer', token);
  return htmlResponse(
...
export async function POST(request) {
  ...
  try {
    ...
    const [rows] = await db.execute(
      'SELECT custm_id, customer_name, email, email_verified FROM customer WHERE LOWER(email) = LOWER(?)',
      [email.trim()]
    );
